...
class CUDA_build_ext(build_ext):
    ...
    def spawn(self, cmd, search_path=1, verbose=0, dry_run=0):
        ...
        if (sys.platform == 'darwin' and len(cmd) >= 2 and cmd[0] == 'nvcc' and
            ...
            temp = 'C:\\Program Files (x86)\\Microsoft Visual Studio 14.0\\vc\\bin' # os.path.dirname(find_executable("cl.exe", PATH))
            print temp
            cmd[:1] = ['nvcc', '--compiler-bindir',
                       temp #os.path.dirname(find_executable("cl.exe", PATH))
                       or cmd[0]]
